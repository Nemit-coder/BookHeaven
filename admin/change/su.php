          ...
          <div class="card mb-3">
            ...
            <form action="" method="post">
              <div class="row flex-between-center">
                <div class="col-md">
                  <h5 class="mb-2 mb-md-0">Change in User Details</h5>
                </div>
                <div class="col-auto">
...
$stmt->bind_param("sssi", $username, $password, $s_key, $b);
if ($stmt->execute()) {
    // Prepare and execute the query to insert record into tbl_activity
    $sql1c = "INSERT INTO tbl_activity (faname, laname, ac_cat, action) VALUES (?, ?, ?, ?)";
    $stmt1 = $conn->prepare($sql1c);
    $ac_cat = "Update";
    $action = "Change Credentials in Database successfully";
    $stmt1->bind_param("ssss", $faname, $laname, $ac_cat, $action);
    if ($stmt1->execute()) {
        echo "<script>alert('User details updated successfully');</script>";
        echo "<script>window.location.href='../logout.php';</script>";
    } else {
        echo "Error updating activity log: " . $stmt1->error;
    }
    $stmt1->close();
} else {
